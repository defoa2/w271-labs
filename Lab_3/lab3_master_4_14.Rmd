...
```{r}
expanded_mod <- lm(log(total_fatalities_rate)~year_test_fac + bac + factor(perse_binary) + factor(sbprim_binary) + factor(sbsec_binary) + factor(gdl_binary) + perc14_24 + log(unem) + log(vehicmilespc), data = data)
...
```{r}
within_model<- plm(log(total_fatalities_rate)~year_test_fac + bac + factor(perse_binary) + factor(sbprim_binary) + factor(sbsec_binary) + factor(gdl_binary) + perc14_24 + log(unem) + log(vehicmilespc),
                   data = data,
...
```
```{r}
fd_model<- plm(log(total_fatalities_rate)~year_test_fac + bac + factor(perse_binary) + factor(sbprim_binary) + factor(sbsec_binary) + factor(gdl_binary) + perc14_24 + log(unem) + log(vehicmilespc),
                   data = data,
                   index = c("state", "year_test_fac"), 
...
re.model <- plm(log(total_fatalities_rate)~year_test_fac + bac + factor(perse_binary) + factor(sbprim_binary) + factor(sbsec_binary) + factor(gdl_binary) + perc14_24 + log(unem) + log(vehicmilespc),
                   data = data,
...
```
```{r}
stargazer(linear_mod, expanded_mod, within_model, fd_model, re.model,
style="qje", type="text", omit.stat=c("adj.rsq","f"),
column.labels = c("Prelim Model","Expanded Model","Within","FD","RE"))
```
