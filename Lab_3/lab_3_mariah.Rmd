...
```{r}
# pdata_yr_data <- pdata %>% 
#   select(c('d80','d81','d82','d83','d84','d85',
...
p5 <- pdata %>%
  filter(as.integer(state) >36 & as.integer(state)<= 48 ) %>%
  ggplot(aes(x = as.Date(year,"%Y"), y = total_fatalities_rate)) +
  geom_line(aes(color = state)) +
  labs(x = "Year",  y = "Fatality rate")+
  geom_label_repel(data = filter(pdata, as.integer(state) <= 12  & year == 1984),
...
p11<-pdata %>%
  ...
  geom_line(aes(color = state)) +
  labs(x = "Year",  y = "Unemployement Rate")+
  geom_label_repel(data = filter(pdata, as.integer(state) > 12 & as.integer(state) <= 24  & year == 1984),
                   aes(label = state), nudge_x = .75,na.rm = TRUE) +
  theme(legend.position = "none")
...
p12<- pdata %>%
  ...
  geom_line(aes(color = state)) +
  labs(x = "Year",  y = "Unemployement Rate")+
  geom_label_repel(data = filter(pdata, as.integer(state) > 24 &as.integer(state) <= 36 & year == 1984),
                   aes(label = state), nudge_x = .75,na.rm = TRUE) +
  theme(legend.position = "none")
...
p13<- pdata %>%
  ...
  geom_line(aes(color = state)) +
  labs(x = "Year",  y = "Unemployement Rate") +
  geom_label_repel(data = filter(pdata, as.integer(state) > 36 & year == 1984),aes(label = state), nudge_x = .75,na.rm = TRUE) +
  theme(legend.position = "none")
