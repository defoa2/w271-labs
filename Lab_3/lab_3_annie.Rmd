...
# year_set_1 <- 80:99
# year_set_2 <- 00:04
# col_name_1 <- paste0("d", year_set_1)
# col_name_2 <- paste0("d0", year_set_2)
# col_name_join <- c(col_name_1, col_name_2)
col_num <- 31:55
years <- 1980:2004
...
# yob_func <- function(column, year){
#   data$year_test <- ifelse(data[, col_num]== 1, year, data$year_test)
# }
...
linear_mod <- lm(totfatrte~year_test_fac, data = data)
summary(linear_mod)
```
...
```{r}
data$gdl_fac <- factor(data$gdl)
data$sbp_fac <- factor(data$sbprim)
data$sbs_fac <- factor(data$sbsecon)
expan_mod <- lm(totfatrte~year_test_fac + bac + perse + sbp_fac + sbs_fac + gdl + perc14_24 + unem + vehicmilespc, data = data)
summary(expan_mod)
```
...
```{r}
within_model<- plm(totfatrte~year_test_fac + bac + perse  + state + sbp_fac + sbs_fac + gdl + perc14_24 + unem + vehicmilespc, data = data,
