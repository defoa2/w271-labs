...
data_piv <- data_piv %>%
  ...
  filter(value == 1) %>% 
  select(-value)
data_piv$speed_70plus <- data_piv$sl70plus
data_piv <- data_piv %>%
  pivot_longer(
    cols = starts_with("sl"),
...
```{r}
pooled_ols <- lm(total_fatalities_rate ~ time(year), data = data_piv)
summary(pooled_ols)
...
```{r}
expanded_mod <- plm(total_fatalities_rate ~ time(year) + blood_alc + perse + sbprim + sbsecon + speed_70plus + gdl + perc14_24 + unem + vehicmilespc, data = data_piv,
index = c("state", "year"),
...
```{r}
within_model<- plm(total_fatalities_rate ~ time(year) + blood_alc + perse + sbprim + sbsecon + speed_70plus + gdl + perc14_24 + unem + vehicmilespc, data = data_piv,
index = c("state", "year"),
...
```{r}
pfatdat <- pdata.frame(data_piv, index=c("state", "year"))


pfatdat <- pfatdat[!duplicated(pfatdat), ]
```


```{r}
re.model <- plm(total_fatalities_rate ~ time(year) + blood_alc + perse + sbprim + sbsecon + speed_70plus + gdl + perc14_24 + unem + vehicmilespc, data = pfatdat,
