...
## Part 1: loading data and creating features
...
```{r,warning=FALSE,echo=FALSE, fig.height=5, fig.width=6}
#State label data 
...
p2 <- pdata %>%
  ...
  labs(x = "Year",  y = "Total Fatality rate", title = "Total Fatality Rate by State")+
...
p3 <- pdata %>%
  ...
  ggplot(aes(x = as.Date(year,"%Y"), y = total_fatalities_rate)) +
  geom_line(aes(color = state)) +
  labs(x = "Year",  y = "Fatality rate", title = " ")+
...
```{r,warning=FALSE,echo=FALSE, fig.height=9, fig.width=6.5}
p6<- pdata %>%
  filter(as.integer(state_no) <= 12 ) %>%
  ggplot(aes(x = as.Date(year,"%Y"), y = total_fatalities_rate)) +
  geom_line() +
...
p9<-pdata %>%
  ...
  labs(x = "Year",  y = "Fatality rate") +
  theme(legend.position = "none") + theme(axis.text.x = element_text(angle=35))
grid.arrange(p2,p3,p4, p5,p6, p7, p8, p9, nrow = 4, ncol = 2)
...
p10<- pdata %>%
  ...
  geom_line(aes(color = state)) +
  labs(x = "Year",  y = "Unemployement Rate", title = "Unemployment by State")+
  geom_label_repel(data = filter(pdata, as.integer(state) <= 12  & year == 1984),
                   aes(label = state), nudge_x = .75,na.rm = TRUE) +
  theme(legend.position = "none") + theme(plot.title = element_text(size=22))
...
p14 <- pdata %>% 
  ...
  labs( title = "Mean Total Fatality Rate vs 1000 Miles Driven PC",
...
```{r, fig.height=2.5, fig.width=5, echo=FALSE}
par(mfrow=c(1,2))
hist(data$perc14_24, main = "Histogram of Percent 14-24", xlab = "Percent 14-24", cex.main = 0.8)
hist((log(data$perc14_24)), main = "Histogram of log of Percent 14-24", xlab = "Log of Percent 14-24", cex.main = 0.75)
```
