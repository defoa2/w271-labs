...
```{r}
data_piv$total_fatalities_rate <- data_piv$totfatrte
data_piv$night_fatalities_rate <- data_piv$nghtfatrte
data_piv$weekend_fatalities_rate <- data_piv$wkndfatrte
data_piv <- data_piv %>%
...
p2<- data_piv %>%
  ...
  geom_label_repel(data = filter(data_piv, as.integer(state) <= 12  & year == 1984),
...
p3<- data_piv %>%
  ...
  geom_line(aes(color = as.character(state))) +
  labs(x = "Year",  y = "Fatality rate")+
  geom_label_repel(data = filter(data_piv, as.integer(state) > 12 & as.integer(state) <= 24  & year == 1984),
                   aes(label = state), nudge_x = .75,na.rm = TRUE) +
  theme(legend.position = "none")
...
p4<- data_piv %>%
  ...
  geom_line(aes(color = as.character(state))) +
  labs(x = "Year",  y = "Fatality rate")+
  geom_label_repel(data = filter(data_piv, as.integer(state) > 24 &as.integer(state) <= 36 & year == 1984),
                   aes(label = state), nudge_x = .75,na.rm = TRUE) +
  theme(legend.position = "none")
...
p5<- data_piv %>%
  ...
  geom_line(aes(color = as.character(state))) +
  labs(x = "Year",  y = "Fatality rate") +
  geom_label_repel(data = filter(data_piv, as.integer(state) > 36 & year == 1984),aes(label = state),
...
data_piv %>%
  group_by(year) %>%
  ggplot(aes(x = year, y = data_piv$vehicmilespc, group = year)) +
  geom_boxplot() +
  labs(x = "Year",  y = "Vehicle miles driven per capita")
```

```{r}
data_piv %>%
  data.frame() %>%
  ggplot(aes(x = state, y = total_fatalities_rate, group=state)) +
  geom_boxplot() +
  labs(x = "States",  y = "Fatality rate")

data_piv %>%
  group_by(state) %>%
  ggplot(aes(x = reorder(state,vehicmilespc), y = vehicmilespc)) +
  geom_boxplot() +
  labs(x = "States",  y = "Vehicle miles driven per capita")
...
```{r}
pooled_ols <- plm(total_fatalities_rate ~ speed_limit, data = data_piv,
index = c("state", "year"),
effect = "individual", model = "pooling")
summary(pooled_ols)

#lin_mod <- lm(total_fatalities_rate ~ seatbelt + speed_limit + blood_alc + perse + statepop + zerotol + minage, data = data_piv)
#summary(lin_mod)
```
...
```{r}
data_piv <- data_piv %>%
  mutate(sl70plus = ifelse(speed_limit > 70, 1, 0))

  
expanded_mod <- plm(total_fatalities_rate ~ speed_limit + blood_alc + perse + sbprim + sbsecon + sl70plus + gdl + perc14_24 + unem + vehicmilespc, data = data_piv,
index = c("state", "year"),
effect = "individual", model = "pooling")
...
```{r}
duplicate_rows <- duplicated(data_piv[, c("state", "year")])
sum(duplicate_rows)
nrow(data_piv)
...
fixed_effects_model <- plm(total_fatalities_rate ~ speed_limit + blood_alc + perse + sbprim + sbsecon + sl70plus + gdl + perc14_24 + unem + vehicmilespc,
                           index =  c("state", "year"),
                           data = data_piv,
                           model = "within")
...
fe_estimate <- coef(fixed_effects_model)["vehicmilespc"]
