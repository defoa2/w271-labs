...
plot <- ggplot()  + geom_line(data = long_forecast_arima, aes(x=index, y = .mean, color = "ARIMA Model")) +
  ...
  geom_text(aes( 0, 420, label = 420, vjust = -1), size = 3) + 
...
The residuals plots show that the ARIMA model trained on the seasonally adjusted CO2 data frame performs better than the non-seasonally adjusted data frame (the in-sample residuals stay closer to 0). 
...
```{r, echo = F, fig.height=3, fig.width=7.5}
...
poly_forecast_plot <- sa_poly_forecast %>%
  ...
  autolayer(sa_train, colour="black")
grid.arrange(autoplot(ns_forecast), autoplot(sa_forecast), poly_forecast_plot, ncol=3)
```
