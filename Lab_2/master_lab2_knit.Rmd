...
CO2_ACF <- df_co2 %>% 
  ACF(value) %>% 
  autoplot() + labs(title = "The ACF for CO2 (ppm)")
...
(Equation 1) $y_{t} = \beta_{0} + \beta_{1}t +\epsilon_{t}$ \
(Equation 2) $y_{t} = \beta_{0} + \beta_{1}t + \beta_{2}t^{2} + \epsilon_{t}$  \
(Equation 3) $y_{t} = \beta_{0} + \beta_{1}t + \beta_{2}t^{2} + \beta_{3}t^{3} + \epsilon_{t}$ \
...
```{r, echo = F, fig.height=1.5, fig.width=4}
...
lin_plot <- df_co2 %>%
  ...
  labs(title = "Fitted Linear Model",
       x = 'time',
       y = 'CO2 (ppm)')+ 
  theme(axis.text.x = element_text(angle=35))+ theme(plot.title = element_text(size=10))
...
par(mfrow=c(2,3))  
plot(residuals_linear, main = "Linear Residuals")
plot(residuals_quadratic, type = "l", main = "Quadratic Residuals")
plot(residuals_poly,type = "l", main = "Polynomial Residuals")
...
geom_line(data=polynomial_model %>% augment(), aes(index,.fitted,color=.model), show.legend = FALSE) + 
  labs(title = "Polynomial Forecast") + theme(axis.text.x = element_text(angle=15))
```
...
autoplot(colour="cornflowerblue") +
autolayer(df_co2, colour="black") +
geom_line(data=sar_bic %>% augment(), aes(index,.fitted,color=.model), show.legend = FALSE)+ 
...
```{r, echo = F, fig.height=2.5, fig.width=4, warning=FALSE}
...
plot <- ggplot()  + geom_line(data = long_forecast_arima, aes(x=index, y = .mean, color = "ARIMA Model")) +
  ...
  geom_hline(yintercept = 420, label = "500") + 
  geom_hline(yintercept = 500, label = "420") + 
  geom_text(aes( 0, 300, label = 420, vjust = -1), size = 3) + 
  geom_text(aes( 0, 500, label = 500, vjust = -1), size = 3) + 
...
### Point Estimate and CI for 420 ppm
...
### Point Estimate and CI for 500 ppm
...
# download and save the file locally as a CSV
# weekly data url
...
# read into from downloaded URL and format dataframe
...
```{r, echo = F, fig.height=2.5, fig.width=6, warning=FALSE}
#Lab Plots 
...
eda1 <- co2_present_mnth %>% 
  gg_season(value, labels="both") + 
  labs(y= "CO2 (ppm)",
       title = "CO2 by Month and Year",
       x = "Month")
 
```

```{r, echo = F, fig.height=2.5, fig.width=7, message=FALSE}
#Seasonal Subseries plot 
...
comp_plot <- lin_forecast_df %>%
  ggplot() + aes(x=index, y = Point.Forecast, color = "Linear Model") + geom_line(show.legend = FALSE) + 
  ...
  geom_line(data=co2_present_mnth, aes(x=mnth, y =value, color = "Actual"), show.legend = FALSE) +
...
```{r, echo = F, fig.height=3, fig.width=7.5, warning=FALSE}
...
sa_poly_forecast <- forecast(poly_trend, h = 104)

poly_pseudo_out_sample <- sa_poly_forecast$.mean - sa_test$value
...
# plot poly pseudo
plot(poly_pseudo_out_sample, xlab = "Time", type = 'l', ylab = "Forecasted - Actual", main = "Pseudo/Out Plot SA")

arima_na_plot <- autoplot(ns_forecast) + ggtitle("ARIMA NAS Forecast") + labs(y = "Co2")
arima_sa_plot <- autoplot(sa_forecast) + ggtitle("ARIMA SA Forecast")
poly_forecast_plot <- sa_poly_forecast %>%
  autoplot(show.legend = FALSE) + 
  autolayer(sa_train, colour="black") + ggtitle("Polynomial Forecast") +
  labs(x = "Time",
       y = "Co2")+ theme(axis.text.x = element_text(angle=25))
grid.arrange(arima_na_plot, arima_sa_plot, poly_forecast_plot, ncol=3)
```

```{r, echo=FALSE}
models <- c("ARIMA NSA", "ARIMA SA", "Polynomial SA")
aic <- c(AIC(arima_ns), AIC(arima_sa), glance(poly_trend)$AIC)
...
Using the non-seasonally adjusted data series, the first time CO2 is expected to be at 420 ppm is October 2026 (with the interval October 2023 - March 2030) and to be 500 ppm in January 2073 for the first time (with the interval April 2068-March 2078). In the year 2122, CO2 is expected to be 747.21 from the seasonally adjusted ARIMA model predictions with the 95% confidence interval -2058.8 to 3553.26. The polynomial model forecasts that in the year 2122 we will reach 1406.12 ppm with the 95% confidence interval 1319.66 to 1492.58. We are not entirely confident in these predictions being accurate, since there is variability in the models, as well as many factors that may speed up or slow down CO2 concentration trends in the next 100 years besides seasonal patterns. It also seems that the confidence intervals are very large for co2 concentration predictions, which might indicate that it is hard to accurately forecast based on the current data.
...
colnames(tab_420) = c('Predicted first time', '95% CI', 'Predicted last time', '95% CI')
rownames(tab_420) = c('ARIMA (NSA)', 'ARIMA (SA)', 'Polynomial Model')
tab_420
```
### Point Estimate and CI for 500 ppm
```{r, echo = F}
...
rownames(tab_2122) = c("ARIMA Model", "Polynomial Model")
colnames(tab_2122) = c("Point Estimate", "95% confidence Interval")
tab_2122
```

# References

Jiménez-de-la-Cuest, Diego and Thorsten Mauritsen. 2019. Emergent constraints on Earth’s transient and equilibrium response to doubled CO2 from post-1970s global warming. Nature Geosciences 12: 902-905. 
