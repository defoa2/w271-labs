...
polynomial_model <- df_co2 %>% model(trend_model = TSLM(formula = value ~ poly(trend(), degree = 4) + season()))
...
# co2_present %>%
#   autoplot(mean)
summary(co2_present$mean)
```

```{r}
CO2_lags <- co2_present %>% 
  gg_lag(mean, geom = 'point') + 
  labs(x = "lag(value,k)", 
...
twob_plot <- real_data %>%
  ggplot() + aes(x=time_index, y = mean) + geom_line() + ggtitle("Actual Co2 Data")
...
In 1997 it was predicted by the ARIMA model that we would cross 420 sometime between 2025 and 2030, however in reality we crossed if between 2020 and 2023.
