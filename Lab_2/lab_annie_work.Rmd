...
polynomial_model <- df_co2 %>% model(trend_model = TSLM(formula = value ~ poly(trend(), degree = 3) + season()))

residuals_poly <- residuals(polynomial_model)
resid3 <- plot(residuals_quadratic, type = "l", main = "Residuals of Quadratic Model")

plot <- df_co2 %>%
  ggplot() + geom_point(aes(x= index, y = value))  + 
  geom_line(aes(x = index, y=fitted(polynomial_model)$.fitted))
grid.arrange()
grid.arrange(resid1, resid2, resid3, ncol=3)
...
mod_names <- c("linear", "log-linear", "quadratic", "polynomial")
mod_aic <- c(AIC(linear_model), AIC(log_linear_model), AIC(quadratic_model), glance(polynomial_model)$AIC)
mod_bic <- c(BIC(linear_model), BIC(log_linear_model), BIC(quadratic_model), glance(polynomial_model)$BIC)
