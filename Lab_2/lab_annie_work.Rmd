...
dist_param = parameters(long_forecast_polynomial$value)

poly_ci <- apply(dist_param,1,  predict_int)
...
lin_forecast <- forecast(linear_model, h=314)
...
# 
# grid.arrange(twoa_plot, twob_plot, ncol = 2)
```
...
#   ggplot() + aes(x=index, y = .mean, color = "ARIMA Model") + geom_line() + 
#   geom_line(data=real_data, aes(x=time_index, y =average, color = "Actual")) +
#   ggtitle("Comparing Actual vs Predicted")
# 
# comp_plot
...
```{r}
accuracy(forecast_polynomial$.mean, real_data$average)
```
