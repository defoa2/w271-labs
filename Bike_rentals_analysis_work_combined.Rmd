...
<!-- H_0 = Peak bike demand is not effected by season. -->
<!--   H_A =  Bike demand will be lowest in the winter, highest in the spring. -->
  
2. H_0 = Bike demand is not related to time of day and temperature combinations.
  H_A =  Bike rental spikes during afternoons with moderate weather (60-75 degrees F)
  
3. H_0 = Temperature does not effect bike rentals.
  H_A =  When the temperature is 60-80 degrees F, there is a spike in bike rentals.
  
4. H_0 = There is consistent trends in bike rentals throughout the day.
  H_A =  Morning and evenings have higher demand for rental bikes (could be because of commutes).
  
5. H_0 = Bike rentals are not effected by holidays.
  H_A = Bike rentals will be in high demand during holidays possibly due to recreational activities.
...
```{r}
# creating new variable time of day from bike
seoul.bike$TempF <- seoul.bike$Temperature.C. * (9/5) + 32

# -1, 103
max(seoul.bike$TempF)
seoul.bike$TempCat <- ifelse(seoul.bike$TempF <= 20, "below 20", seoul.bike$TempF)
seoul.bike$TempCat <- ifelse(seoul.bike$TempF > 20, "21-40", seoul.bike$TempCat)
seoul.bike$TempCat <- ifelse(seoul.bike$TempF > 40, "41-60", seoul.bike$TempCat)
seoul.bike$TempCat <- ifelse(seoul.bike$TempF > 60, "61-80", seoul.bike$TempCat)
seoul.bike$TempCat <- ifelse(seoul.bike$TempF > 80, "81-100", seoul.bike$TempCat)
seoul.bike$TempCat <- ifelse(seoul.bike$TempF> 100, "over 100", seoul.bike$TempCat)
seoul.bike$TempCat <- as.factor(seoul.bike$TempCat)
summary(seoul.bike$TempCat)
dim(seoul.bike)
```
```{r}
tempcat_bar <- ggplot(data=seoul.bike, aes(x=TempCat, y=Rented.Bike.Count)) + geom_bar(stat = 'identity')
tempcat_bar
```
...
```{r}
hist(seoul.bike$Solar.Radiation..MJ.m2.)
```
...
```{r}
p <- ggplot(aes(x= seoul.bike$'Temperature(°C)', y = seoul.bike$'Rented Bike Count', color = Seasons), data = seoul.bike) + geom_point()
p
```

```{r}
ggplot(seoul.bike, aes(x = seoul.bike$Seasons, y = seoul.bike$`Rented Bike Count`, fill = seoul.bike$time.of.day)) +
...
```{r}
ggplot(seoul.bike, aes(x = seoul.bike$Holiday, y = seoul.bike$`Rented Bike Count`)) +
  geom_bar(stat = "identity", position = "dodge") +
...
```{r}
pos.mod <- glm(formula = Rented.Bike.Count ~ time.of.day +  TempCat + Seasons +Holiday + Functioning.Day, family = poisson(link = "log"), data = seoul.bike)
summary(pos.mod)
```
...
```{r}
pos.mod2 <- glm(formula = Rented.Bike.Count ~ time.of.day +  TempCat + Seasons +Holiday + Functioning.Day + Solar.Radiation..MJ.m2. + Rainfall.mm. + Snowfall..cm., family = poisson(link = "log"), data = seoul.bike)
summary(pos.mod2)
...
```{r}
pos.mod3 <- glm(formula = Rented.Bike.Count ~ time.of.day +  TempCat + Seasons +Holiday + Functioning.Day 
                + Solar.Radiation..MJ.m2. + Rainfall.mm. + Snowfall..cm. +
                  TempCat:Seasons + Holiday:time.of.day,
                family = poisson(link = "log"), data = seoul.bike)
summary(pos.mod3)
```
